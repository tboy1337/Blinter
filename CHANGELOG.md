...
- **P024**/**P006**/**P003**: a command after an `IF` predicate or a separator, as in `if defined FLAG setlocal`, is a command the line runs, so `SETLOCAL`/`ENDLOCAL` are no longer undercounted; a quoted operand holding a space stays one token, a caret-escaped `^&` is data, and a single `|` is a pipe rather than a separator (follow-up to [#40](https://github.com/tboy1337/Blinter/issues/40))
...
- Pinned batch-spec language SSOT to v0.70.0 (unquoted `)` inside a command group closes the group; leftover text is a syntax error)
