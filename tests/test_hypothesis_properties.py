...
# Helper to create fuzz-compatible settings
# When fuzzing, we don't suppress health checks as hypofuzz needs full control
_IS_FUZZING = os.getenv("HYPOTHESIS_PROFILE") == "fuzzing"


def fuzz_settings(**kwargs: object) -> settings:
    """Create settings that are compatible with both regular testing and fuzzing."""
    if _IS_FUZZING:
        # When fuzzing, only use deadline and max_examples - no suppress_health_check
        return settings(deadline=None, max_examples=1000, **kwargs)
    else:
        # When testing normally, suppress slow health check
        return settings(
...
def blinter_config_strategy(draw: st.DrawFn) -> BlinterConfig:
    ...
    min_severity = draw(st.one_of(st.none(), rule_severity_strategy()))
...
class TestConfigurationProperties:
    ...
    def test_set_min_severity_invalid(self, severity_str: str) -> None:
        ...
        config = BlinterConfig()
        # Should not raise an exception, just log a warning (suppressed for HypoFuzz)
        logger = logging.getLogger("blinter")
        original_level = logger.level
        logger.setLevel(logging.ERROR)  # Suppress WARNING level logs
        try:
            _set_min_severity(config, severity_str)
        finally:
...
class TestCodeAnalysisProperties:
    ...
    def test_calculate_exit_paren_depth_deterministic(
        ...
        assert result1 == result2
        assert isinstance(result1, int)
        # Depth can be negative for unbalanced parentheses (more closing than opening)
        assert -50 <= result1 <= 50  # Reasonable bounds for test data
