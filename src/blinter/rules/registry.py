...
RULES: Dict[str, Rule] = {
    ...
    "E003": Rule(
        ...
        severity=RuleSeverity.ERROR,
        explanation="IF statement has improper spacing or syntax that will prevent correct execution, including a single = where == is required and a missing space before (",
        recommendation='Use proper spacing: IF "condition" == "value" ( and IF EXIST/DEFINED/ERRORLEVEL forms without a single equals',
    ),
    "E004": Rule(
        code="E004",
    ...
    "E007": Rule(
        ...
        recommendation='Use IF NOT DEFINED VAR or IF "%VAR%"=="" for empty variable checks',
    ),
    "E008": Rule(
        code="E008",
    ...
    "E043": Rule(
        code="E043",
        name="Double-colon comment inside parenthesized block",
        severity=RuleSeverity.ERROR,
        explanation="Inside a parenthesized block, cmd.exe treats :: as a label rather than a comment; labels in blocks break block parsing",
        recommendation="Use REM for comments inside IF/FOR parenthesized blocks",
    ),
    "E044": Rule(
        code="E044",
        name="ELSE on a new line",
        severity=RuleSeverity.ERROR,
        explanation="ELSE must share the line with the closing parenthesis of the IF block; an ELSE that starts a new line is not paired and runs with silent wrong control flow",
        recommendation="Write ) ELSE ( on the same line as the close of the IF block",
    ),
    "E045": Rule(
        code="E045",
        name="Invalid internal command switch or argument",
        severity=RuleSeverity.ERROR,
        explanation="Several cmd.exe internal commands accept only a documented set of slash-switches or no arguments at all; unknown switches and extra arguments fail at runtime",
        recommendation="Use only documented switches (for example IF /I, CD /D, SET /A /P, EXIT /B, FOR /D /F /L /R, DATE /T, TIME /T, SHIFT /0-/8) and omit arguments for CLS, PAUSE, VER, POPD, and ENDLOCAL",
    ),
    "E046": Rule(
        code="E046",
        name="Malformed stream redirection",
        severity=RuleSeverity.ERROR,
        explanation="Merging stderr and stdout requires 2>&1 or 1>&2; a space before & (2> &1) is a live syntax error. 2>>&1 is valid append-duplication",
        recommendation="Write 2>&1 or 1>&2 with no space before &, and place 2>&1 at the end of the command",
    ),
    "E047": Rule(
        code="E047",
        name="Unescaped specials in FOR /F command or ECHO in a block",
        severity=RuleSeverity.ERROR,
        explanation="Unescaped & < > | inside a FOR /F IN ('command') string, or unescaped parentheses in ECHO inside a parenthesized block, terminate the loop or block early",
        recommendation="Escape as ^& ^< ^> ^| and ^( ^) inside those contexts",
    ),
    ...
    "S029": Rule(
        code="S029",
        name="@ prefix after the first line",
        severity=RuleSeverity.STYLE,
        explanation="Prefixing commands with @ after line 1 hides individual commands without documenting the script-wide echo policy; @ECHO OFF at the start is the conventional way to suppress command echo",
        recommendation="Use @ECHO OFF (or @ECHO ON) on the first line and omit @ on later commands except when a single line must stay silent",
    ),
    "S030": Rule(
        code="S030",
        name="Redundant colon on device name",
        severity=RuleSeverity.STYLE,
        explanation="Device names such as NUL, CON, and PRN do not need a trailing colon; NUL: is a redundant form",
        recommendation="Write NUL, CON, or PRN without a colon",
    ),
    ...
    "SEC001": Rule(
        ...
        explanation="SET /P input stored in a variable and later used unquoted in a command can inject extra commands if the user types & | > or similar",
        recommendation="Quote every use of SET /P variables, or validate with IF DEFINED and character checks before expanding them",
    ),
    "SEC002": Rule(
        code="SEC002",
    ...
    "SEC014": Rule(
        ...
        severity=RuleSeverity.SECURITY,
        explanation="Unquoted batch arguments %1-%9, %*, and %~1 are filled at runtime and can inject extra commands when they contain & | > < or similar",
        recommendation='Always quote arguments, for example "%~1" or "%*"',
    ),
    "SEC015": Rule(
        code="SEC015",
    ...
    "SEC025": Rule(
        code="SEC025",
        name="Unquoted current-directory expansion",
        severity=RuleSeverity.SECURITY,
        explanation="Unquoted %CD%, %__CD__%, or !CD! can insert extra commands when the current directory contains & | > < or similar",
        recommendation='Always quote directory expansions, for example "%CD%"',
    ),
    ...
    "W017": Rule(
        ...
        severity=RuleSeverity.WARNING,
        explanation="IF ERRORLEVEL 0 is always true for native commands because ERRORLEVEL n means >= n. IF NOT ERRORLEVEL 0 is almost always false except for negative exit codes. IF %ERRORLEVEL% NEQ 1 matches any value except 1 (0,2,3...), while NOT ERRORLEVEL 1 only matches values less than 1 (i.e., 0).",
        recommendation="Use IF ERRORLEVEL 1 for traditional failure checks, IF %ERRORLEVEL% NEQ 0 or IF %ERRORLEVEL% EQU 0 for explicit comparisons, and avoid IF ERRORLEVEL 0, IF NOT ERRORLEVEL 0, and IF %ERRORLEVEL% NEQ 1",
    ),
    "W018": Rule(
        code="W018",
    ...
    "W044": Rule(
        ...
        severity=RuleSeverity.WARNING,
        explanation='A space before the equals sign in SET is part of the variable name (set X =Hello names the variable "X "), so later %X% expansions miss it',
        recommendation='Use SET var=value or SET "var=value" with no space before =',
    ),
    "W045": Rule(
        code="W045",
    ...
    "W064": Rule(
        code="W064",
        name="ALLUSERPROFILE environment-variable typo",
        severity=RuleSeverity.WARNING,
        explanation="ALLUSERPROFILE is a common misspelling of ALLUSERSPROFILE and expands empty",
        recommendation="Use %ALLUSERSPROFILE%",
    ),
    "W065": Rule(
        code="W065",
        name="Wildcard in IF string comparison",
        severity=RuleSeverity.WARNING,
        explanation="IF == comparisons are literal string matches; * and ? are not glob wildcards there",
        recommendation="Use FOR, FINDSTR, or an explicit list of comparisons instead of * or ? in IF ==",
    ),
    "W066": Rule(
        code="W066",
        name="2>&1 not at the end of the command",
        severity=RuleSeverity.WARNING,
        explanation="Stream merge 2>&1 applies to the redirection state at that point; placing it before other redirects often fails to capture stderr as intended",
        recommendation="Put 2>&1 at the end of the command, for example DIR >log.txt 2>&1",
    ),
    "W067": Rule(
        code="W067",
        name="SET of a system environment variable",
        severity=RuleSeverity.WARNING,
        explanation="Assigning PATH, USERNAME, USERPROFILE, TEMP, TMP, APPDATA, and similar system variables changes the process environment for the rest of the script and child processes",
        recommendation="Prefer a distinct variable name, or restore the original value if a temporary override is required",
    ),
    "W068": Rule(
        code="W068",
        name="Percent expansion of a variable set in the same parenthesized block",
        severity=RuleSeverity.WARNING,
        explanation="cmd.exe expands %var% when a parenthesized block is parsed, so SET inside the block does not update later %var% on that same parse",
        recommendation="Use !var! with SETLOCAL EnableDelayedExpansion, or SET the variable before the block",
    ),
    "W069": Rule(
        code="W069",
        name="TIME expansion without replacing the leading space",
        severity=RuleSeverity.WARNING,
        explanation="Before 10:00, %TIME% begins with a space rather than a zero, which breaks concatenations, filenames, and colon/dot stripping",
        recommendation="Substitute the leading space with %TIME: =0% before using TIME in names or further replacements",
    ),
    "W070": Rule(
        code="W070",
        name="SET PATH or PROMPT chained with AND in a .bat file",
        severity=RuleSeverity.WARNING,
        explanation="In .bat files SET, PATH, and PROMPT do not reset ERRORLEVEL to 0 on success, so cmd1 && cmd2 may skip the second command; .cmd files do reset it",
        recommendation="Avoid && after SET, PATH, or PROMPT in .bat files; use a separate line, or rename the script to .cmd",
    ),
}
