...
def _for_f_operand_text(line: str) -> str:
    ...
    A backquoted command carries parentheses of its own, as in
    ``(Invoke-WebRequest ...).Headers``, so the operand ends at the
    parenthesis that balances the opener outside quoting, not at the first
    one. A line whose parentheses never balance falls back to the text up to
    that first parenthesis.
    """
