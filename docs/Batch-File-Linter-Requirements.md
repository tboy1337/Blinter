...
Scripts may suppress findings using documentation comments:

- `REM LINT:IGNORE` or `:: LINT:IGNORE` — suppress all rules on the **next** line
- `REM LINT:IGNORE E001,W001` — suppress specific rule codes on the **next** line
- `REM LINT:IGNORE-LINE` or `REM LINT:IGNORE-LINE E001` — suppress on the **same** line

`::` comment syntax is also supported for all forms above.
